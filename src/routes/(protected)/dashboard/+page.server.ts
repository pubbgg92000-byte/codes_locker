...
export const load = async ({ locals, url }) => {
  ...
  const [brandsResult, typesResult] = await Promise.all([
    locals.supabase.from('coupon_brands').select('id, name').eq('user_id', user!.id).order('name'),
    locals.supabase.from('coupon_types').select('id, name').eq('user_id', user!.id).order('name')
  ]);
  ...
  if (filters.status === 'available') couponsQuery = couponsQuery.eq('is_used', false);
  if (filters.status === 'used') couponsQuery = couponsQuery.eq('is_used', true);
  if (filters.expiry === 'expired') couponsQuery = couponsQuery.lt('expires_at', today());
  if (filters.expiry === 'active') couponsQuery = couponsQuery.or(`expires_at.is.null,expires_at.gte.${today()}`);
  if (filters.expiry === 'none') couponsQuery = couponsQuery.is('expires_at', null);
  if (filters.expiry === 'soon') {
    const nextWeek = new Date();
    nextWeek.setDate(nextWeek.getDate() + 7);
    couponsQuery = couponsQuery.gte('expires_at', today()).lte('expires_at', nextWeek.toISOString().slice(0, 10));
  }
  if (filters.search) {
    const search = filters.search.replaceAll(',', ' ');
    couponsQuery = couponsQuery.or(`title.ilike.%${search}%,code.ilike.%${search}%,value_notes.ilike.%${search}%`);
  }
...
export const actions = {
  addCoupon: async ({ request, locals }) => {
    ...
    try {
      const brandId = await findOrCreateBrand(locals.supabase, user!.id, String(form.get('brand') ?? 'General'));
      const typeId = await findOrCreateType(locals.supabase, user!.id, String(form.get('type') ?? 'Amount off'));
      ...
      return fail(400, { error: error instanceof Error ? error.message : 'Could not save coupon.' });
    }
  },
  ...
  bulkAdd: async ({ request, locals }) => {
    ...
    try {
      ...
      return fail(400, { error: error instanceof Error ? error.message : 'Could not import coupons.' });
    }
  },
  ...
  updateCoupon: async ({ request, locals }) => {
    ...
    try {
      ...
      return fail(400, { error: error instanceof Error ? error.message : 'Could not update coupon.' });
    }
  },
